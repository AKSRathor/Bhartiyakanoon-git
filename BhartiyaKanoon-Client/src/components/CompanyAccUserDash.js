        ...
        if (file) {
            try {
                ...
                const tx = await signer.sendTransaction({
                    to: "0x6a657360e4C5a26b7457CFE11c008C4388sde7205 <- wrong address",
                ...
                await setTimeout(async() => {
                    ...
                    setTimeout(async() => {
                        ...
                        const firebaseConfig = {
                            apiKey: "firebase_api_key(hidden)",
                            authDomain: "bkanoon-abc.firebaseapp.com",
                            projectId: "bkanoon-abc",
                            storageBucket: "bkanoon-abc.appspot.com",
                            messagingSenderId: "678216894493",
                            appId: "app_id_hidden",
                            measurementId: "Measurement_id_Hidden"
                        };
        ...
        try {
            ...
            const items = await Promise.all(allCerti.map(async i => {
                ...
                if (i.tokenId.toNumber() === Number(certToken) && !check) {
                    check = true;
                    const tx = await signer.sendTransaction({
                        to: "0x6a657360e4C5a26b7457CFE11c008C43884323a05 <- wrong address",
                        value: ethers.utils.parseEther("0.0000069")
                    });
